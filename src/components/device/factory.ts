...
export default class DeviceFactory {
  ...
  /**
   * Function that returns an upgrader object for a given devInstance
   *
   * @param devInstance The device instance retrived from IHuddlyDeviceAPI
   * @param cameraDiscoveryEmitter The eventEmitter that will allow the upgrader object to see attach/detach events
   * @param preferredDeviceApi The main IHuddlyDeviceAPI used for communicating
   * with the camera
   * @param secondaryDeviceApis Fallback IHuddlyDeviceAPI-s in case the
   * main interface does not work
   * @param logger Logger
   */
  static async getDeviceUpgrader(
    cameraManager: IDeviceManager,
    cameraDiscoveryEmitter: EventEmitter,
    preferredDeviceApi: IHuddlyDeviceAPI,
    secondaryDeviceApis: Array<IHuddlyDeviceAPI>,
    logger: any): Promise<IDeviceUpgrader> {
    if (cameraManager instanceof HuddlyGo) {
      const hidApi = await this.getHIDInterface(cameraManager, preferredDeviceApi, secondaryDeviceApis);
      return new HuddlyGoUpgrader(cameraManager, cameraDiscoveryEmitter, hidApi, logger);
    } else if (cameraManager instanceof Boxfish) {
      return new BoxfishUpgrader(cameraManager, cameraDiscoveryEmitter, logger);
    }
    throw new Error('Upgrader not implemented');
  }
  ...
  static async getDevice(
    ...
    switch (productId) {
      case HUDDLY_GO_PID:
        // const hidInterface = await this.getHIDInterface(devInstance, preferredDeviceApi, secondaryDeviceApis);
        device = new HuddlyGo(devInstance, transport, uvcControlInterface, logger);
        break;
      case HUDDLY_BOXFISH_PID:
        device = new Boxfish(devInstance, transport, uvcControlInterface, logger);
        break;
      default:
